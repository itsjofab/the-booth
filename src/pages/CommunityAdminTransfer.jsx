...
export default function CommunityAdminTransfer() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (error) {
        console.error("LOAD TRANSFER MEMBERS ERROR:", error);
        setMembers([]);
        setLoading(false);
        return;
      }
  ...
  return (
    <div style={styles.container}>
      <div style={styles.headerRow}>
        ...
        <h2 style={styles.title}>Transfer ownership</h2>
      </div>

      <p style={styles.subtitle}>{community?.name || "Community"}</p>

      <p style={styles.infoText}>
        Choose another member to make admin. After another admin exists, you can delete your account.
      </p>
      ...
      <div style={styles.list}>
        {paginated.map((member) => (
          <div key={member.id} style={styles.row}>
            ...
            >
              {member.role === "admin" ? "Already admin" : "Make admin"}
            </button>
          </div>
        ))}
      </div>
